...
def fetch_all_weather():
    ...
    # Catch all exceptions to ensure fallback always triggers
        ...
        # Return a safe empty structure so the frontend doesn't crash on null properties
        return {
            "forecasts": None,
            "alertsCork": {"alerts": []},
            "alertsKerry": {"alerts": []},
        ...
        function loadWeatherData() {
            try {
                if (!PRELOADED_WEATHER_DATA && !PRELOADED_TIDE_DATA) {
                    ...
                    throw new Error('All weather and tide data is missing.');
                }

                const { forecasts, alertsCork, alertsKerry } = PRELOADED_WEATHER_DATA || {};
                const tides = PRELOADED_TIDE_DATA || {};
                
                // --- NEW: Robust Check for Weather Availability ---
                // We check if forecasts exist and have at least 2 locations (Cork, Kerry)
                let isWeatherAvailable = false;
                let cork, kerry;
                
                if (forecasts && forecasts.length >= 2 && forecasts[0] && forecasts[1]) {
                ...
                if (isWeatherAvailable) {
                    ...
                    // --- Weather Unavailable Summary ---
                     summaryHtml += `
                        <div class="text-center bg-gray-50 p-4 rounded-xl border-2 border-gray-200 shadow-sm opacity-70">
                            <strong class="block text-xl font-bold text-gray-700">Cork Weather</strong> 
                            <span class="block text-lg font-bold text-gray-500 mt-1">N/A</span>
                        </div>
                        <div class="text-center bg-gray-50 p-4 rounded-xl border-2 border-gray-200 shadow-sm opacity-70">
                            <strong class="block text-xl font-bold text-gray-700">Kerry Weather</strong> 
                            <span class="block text-lg font-bold text-gray-500 mt-1">N/A</span>
                        </div>
                    `;
                }
                ...
                for (let i = 0; i < 3; i++) {
                    if (isWeatherAvailable) {
                        ...
                        // --- Weather Unavailable Detail ---
                        detailHtml += `
                            <div class="bg-gray-50 border-2 border-gray-200 p-4 rounded-xl shadow-sm opacity-60">
                                <strong class="text-gray-500 font-bold text-base">${dayLabels[i]}</strong>
                                <div class="text-xs mt-2 text-center py-4 font-semibold text-gray-400">
                                    System Unavailable
                                </div>
                            </div>
                        `;
                ...
                // --- Tides Column (Should work even if weather is down) ---
                const corkTides = tides.Cork || {};
                const kerryTides = tides.Kerry || {}; 
